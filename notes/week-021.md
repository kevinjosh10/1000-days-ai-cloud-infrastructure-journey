# WEEK 21 NOTES — INFRASTRUCTURE AS CODE: TERRAFORM I

**Week 21 | Phase 2: Infrastructure as Code (Terraform I)**

## 🎯 Notes Objective

Consolidate Week 21's deep dive into HashiCorp Terraform, documenting the transition from manual AWS ClickOps to strict, version-controlled declarative infrastructure through a detailed notes document.

### 1️⃣ HCL Syntax & The Terraform Lifecycle

**Technical Deep Dive:** Mastered the declarative syntax of HashiCorp Configuration Language and the core `init -> plan -> apply -> destroy` workflow.
...
**Key Takeaway:** Never apply changes blindly. `terraform plan` is the ultimate safety net against destructive API calls.

---

### 2️⃣ Dynamic Configurations (Variables & Data Sources)

**Technical Deep Dive:** Eliminated hardcoded strings by injecting input variables, exporting outputs, and dynamically querying the AWS API for real-time data (like AMI IDs).
...
**Key Takeaway:** Hardcoding destroys reusability. Strict variable typing ensures multi-environment scalability.

---

### 3️⃣ Remote State Management

**Technical Deep Dive:** Migrated the vulnerable local `terraform.tfstate` to a highly available Amazon S3 backend, secured by DynamoDB state locking.
...
**Key Takeaway:** Local state is for hobbyists. Remote state and locking are mandatory for any real engineering team.

---

### 4️⃣ Programmatic HCL Logic (`count` & `for_each`)

**Technical Deep Dive:** Implemented loops to provision multiple instances instantly, and conditional ternary operators to toggle resources based on the environment.
...
**Key Takeaway:** A single line of logical code can scale a cluster from 2 nodes to 1,000 nodes seamlessly.

---

### 5️⃣ DRY Code (`locals` & `dynamic` blocks)

**Technical Deep Dive:** Refactored codebase repetition using local values for standardized tagging and dynamic blocks for nested Security Group rules.
...
**Key Takeaway:** Don't Repeat Yourself. Computing values internally prevents drift and human error in mandatory configurations.

---

### 6️⃣ Safeguards & Imports

**Technical Deep Dive:** Learned to protect production databases using `prevent_destroy`, and successfully pulled rogue manual infrastructure under Terraform control via `terraform import`.
...
**Key Takeaway:** Manual infrastructure is technical debt. If it's not in the code, it doesn't exist.

---

### 7️⃣ Project 1: The Production AWS VPC

**Technical Deep Dive:** Architected a complete, multi-tier Virtual Private Cloud (Public/Private Subnets, IGW, NAT, Route Tables) entirely in code.
...
**Key Takeaway:** Building a secure network perimeter via code solidifies everything learned in Phase 1 and Phase 2.
