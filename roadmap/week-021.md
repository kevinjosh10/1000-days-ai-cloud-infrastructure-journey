# WEEK 21 ROADMAP — INFRASTRUCTURE AS CODE: TERRAFORM I

**Week 21 | Phase 2: Infrastructure as Code (Terraform I)**

## 🎯 Roadmap Objective

Consolidate Week 21's deep dive into HashiCorp Terraform, documenting the transition from manual AWS ClickOps to strict, version-controlled declarative infrastructure through a detailed roadmap document.

### 1️⃣ HCL Syntax & The Terraform Lifecycle

**Execution Plan:** Mastered the declarative syntax of HashiCorp Configuration Language and the core `init -> plan -> apply -> destroy` workflow.

- [x] Architect HCL Blocks
- [x] Run `terraform plan`
- [x] Apply changes to AWS

**Milestone Goal:** Never apply changes blindly. `terraform plan` is the ultimate safety net against destructive API calls.

---

### 2️⃣ Dynamic Configurations (Variables & Data Sources)

**Execution Plan:** Eliminated hardcoded strings by injecting input variables, exporting outputs, and dynamically querying the AWS API for real-time data (like AMI IDs).

- [x] Architect HCL Blocks
- [x] Run `terraform plan`
- [x] Apply changes to AWS

**Milestone Goal:** Hardcoding destroys reusability. Strict variable typing ensures multi-environment scalability.

---

### 3️⃣ Remote State Management

**Execution Plan:** Migrated the vulnerable local `terraform.tfstate` to a highly available Amazon S3 backend, secured by DynamoDB state locking.

- [x] Architect HCL Blocks
- [x] Run `terraform plan`
- [x] Apply changes to AWS

**Milestone Goal:** Local state is for hobbyists. Remote state and locking are mandatory for any real engineering team.

---

### 4️⃣ Programmatic HCL Logic (`count` & `for_each`)

**Execution Plan:** Implemented loops to provision multiple instances instantly, and conditional ternary operators to toggle resources based on the environment.

- [x] Architect HCL Blocks
- [x] Run `terraform plan`
- [x] Apply changes to AWS

**Milestone Goal:** A single line of logical code can scale a cluster from 2 nodes to 1,000 nodes seamlessly.

---

### 5️⃣ DRY Code (`locals` & `dynamic` blocks)

**Execution Plan:** Refactored codebase repetition using local values for standardized tagging and dynamic blocks for nested Security Group rules.

- [x] Architect HCL Blocks
- [x] Run `terraform plan`
- [x] Apply changes to AWS

**Milestone Goal:** Don't Repeat Yourself. Computing values internally prevents drift and human error in mandatory configurations.

---

### 6️⃣ Safeguards & Imports

**Execution Plan:** Learned to protect production databases using `prevent_destroy`, and successfully pulled rogue manual infrastructure under Terraform control via `terraform import`.

- [x] Architect HCL Blocks
- [x] Run `terraform plan`
- [x] Apply changes to AWS

**Milestone Goal:** Manual infrastructure is technical debt. If it's not in the code, it doesn't exist.

---

### 7️⃣ Project 1: The Production AWS VPC

**Execution Plan:** Architected a complete, multi-tier Virtual Private Cloud (Public/Private Subnets, IGW, NAT, Route Tables) entirely in code.

- [x] Architect HCL Blocks
- [x] Run `terraform plan`
- [x] Apply changes to AWS

**Milestone Goal:** Building a secure network perimeter via code solidifies everything learned in Phase 1 and Phase 2.
