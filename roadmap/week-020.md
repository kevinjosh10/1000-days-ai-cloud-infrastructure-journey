# WEEK 20 ROADMAP — PHASE 2 ORIENTATION: INFRASTRUCTURE AS CODE

**Week 20 | Phase: Phase 2 Orientation (Infrastructure as Code & Orchestration)**

## 🎯 Roadmap Objective

Consolidate Week 20's transition from AWS Cloud theoretical study into hands-on Phase 2 Engineering through a detailed roadmap document.

### 1️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 2️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 3️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 4️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 5️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 6️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 7️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 8️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 9️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 10️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 11️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 12️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 13️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 14️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 15️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 16️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 17️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 18️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 19️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 20️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 21️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 22️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 23️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 24️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 25️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 26️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 27️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 28️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 29️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 30️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 31️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 32️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 33️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 34️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 35️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 36️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 37️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 38️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 39️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 40️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 41️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 42️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 43️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 44️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 45️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 46️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 47️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 48️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 49️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 50️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 51️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 52️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 53️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 54️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 55️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 56️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 57️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 58️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 59️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 60️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 61️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 62️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 63️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 64️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 65️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 66️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 67️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 68️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 69️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 70️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 71️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 72️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 73️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 74️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 75️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 76️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 77️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 78️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 79️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 80️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 81️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 82️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 83️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 84️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 85️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 86️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 87️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 88️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 89️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 90️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 91️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 92️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 93️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 94️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 95️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 96️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 97️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 98️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 99️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 100️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 101️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 102️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 103️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 104️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 105️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 106️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 107️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 108️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 109️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 110️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 111️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 112️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 113️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 114️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 115️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 116️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 117️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 118️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 119️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 120️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 121️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 122️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 123️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 124️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 125️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 126️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 127️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 128️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 129️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 130️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 131️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 132️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 133️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 134️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 135️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 136️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 137️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 138️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 139️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 140️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 141️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 142️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 143️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 144️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 145️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 146️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 147️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 148️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 149️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 150️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 151️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 152️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 153️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 154️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 155️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 156️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 157️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 158️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 159️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 160️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 161️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 162️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 163️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 164️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 165️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 166️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 167️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 168️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 169️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 170️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 171️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 172️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 173️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 174️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

### 175️⃣ Phase 2 Transition

**Execution Plan:** Shifted focus from AWS SAA-C03 multiple-choice theory to writing actual Infrastructure as Code.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** ClickOps is dead. All future infrastructure will be written declaratively, version-controlled, and peer-reviewed.

---

### 176️⃣ Terraform Fundamentals

**Execution Plan:** Installed the Terraform CLI, initialized the AWS Provider, and studied HCL (HashiCorp Configuration Language).

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Terraform acts as the bridge to AWS. The Write -> Plan -> Apply lifecycle prevents catastrophic deployment errors.

---

### 177️⃣ Docker & Containerization

**Execution Plan:** Installed Docker Desktop, analyzed Containers vs VMs, and deployed isolated Nginx and Python environments.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers share the host kernel, enabling millisecond boot times and massive resource efficiency compared to heavy VMs.

---

### 178️⃣ Dockerfiles & Volumes

**Execution Plan:** Engineered custom Dockerfiles (FROM, WORKDIR, COPY, CMD) and established data persistence via bind mounts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Containers are ephemeral; any critical data must be mapped to a host volume to survive a container restart.

---

### 179️⃣ Kubernetes Orchestration

**Execution Plan:** Installed kubectl and Minikube, deployed Pods, and exposed services using NodePorts.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Docker runs the application, but Kubernetes orchestrates the fleet, ensuring self-healing, scaling, and rolling updates.

---

### 180️⃣ Workstation & IAM Security

**Execution Plan:** Secured local AWS credentials by creating a dedicated IAM `terraform` profile with programmatic access.

- [x] Install CLI Tools
- [x] Configure AWS IAM Profile
- [x] Execute Hello World Container

**Milestone Goal:** Never use the root AWS account for IaC. Always isolate programmatic execution through dedicated, auditable IAM service users.

---

